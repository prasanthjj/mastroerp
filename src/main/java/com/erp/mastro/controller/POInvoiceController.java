...
public class POInvoiceController {
    ...
    /**
     * Method to get po invoice list
     *
     * @param model
     * @return po invoice list
     */
    @GetMapping("/getPurchaseInvoiceList")
    public String getPurchaseInvoiceList(Model model) {
        ...
        try {
            ...
            List<POInvoice> poInvoiceList = currentBranch.getPoInvoiceSet().stream()
                    .filter(poInvoiceData -> (null != poInvoiceData))
                    .sorted(Comparator.comparing(
                            POInvoice::getId).reversed())
                    .collect(Collectors.toList());
            model.addAttribute("poInvoiceList", poInvoiceList);

            return "views/PurchaseInvoiceList";
    ...
    @RequestMapping(value = "/getPurchaseInvoicePreview", method = RequestMethod.GET)
    public String getPurchaseInvoicePreview(HttpServletRequest request, @RequestParam("poInvoiceId") Long poInvoiceId, Model model) {
        ...
        try {
            ...
            return "views/POInvoiceView";

        } catch (Exception e) {
            MastroLogUtils.error(POInvoiceController.class, "Error occured while getting po invoice view", e);
            throw e;
        }
    }
}
